...
The collection was renamed from The Gulf Coast Collection to The Binford Collection after launch. The display name lives in one place, `src/lib/site.ts` (`SITE_NAME`), and the owner's mark, a lone-star compass lockup over the word TEXANA, is `src/components/Logo.astro` (full lockup crowning the entry hall; the emblem alone is `public/favicon.svg`). Accession numbers keep their `GCC.` prefix on purpose: they are catalog identifiers and shareable URLs, not branding.

The homepage entry and exit copy is no longer the verbatim prototype extraction: the AI-features addendum supersedes the entry subtitle, entry wall text, first exit paragraph, and colophon with growth-proof strings (no object counts, room counts, or date ranges to maintain). The overrides live at the top of `scripts/extract-prototype.mjs`, which still extracts everything else, including the seed migration, byte-identically.
...
5. First deploy. The presence of `@netlify/database` provisions the database, and migrations in `netlify/database/migrations` (schema + seed rows) apply automatically before the deploy is published. Note the platform applies migrations after the build step, so any build that ships new migrations prerenders against a database one schema version behind: no tables at all on the first deploy (Postgres 42P01), missing columns or tables on later schema-shipping deploys (42703/42P01). The build detects exactly those states, warns, and renders the public pages from the committed seed extraction for that one deploy; the migrations apply at its end, so trigger one more deploy afterward to render from the database again. Any other database failure still fails the build.
...
All AI features run on the current Claude Sonnet model (`claude-sonnet-5`) through the official `@anthropic-ai/sdk`, using the owner's `ANTHROPIC_API_KEY`. The model name and every per-call limit (max tokens, effort, web search uses, caps) live in `src/lib/ai-config.ts`, not scattered through code. Model and tool choices were verified against current Anthropic documentation (July 2026): adaptive thinking is this model's default (no thinking configuration is sent), sampling parameters are not sent (rejected on this model), structured JSON uses `output_config.format`, and the current web search tool variant for it is `web_search_20260209`. Netlify synchronous functions allow 60 seconds (verified against current Functions docs), which bounds the research call; web search is capped at 5 uses per run.

- **Intake** (`/admin/intake`): upload up to four photographs of a new piece with optional notes. A server function sends them to the model with several existing labels from the database as voice examples; it proposes title, date, maker, medium, object type, meta line, and a draft label in the collection's register, with an instruction (and a server-side backstop) that no em dashes appear. The proposal is presented for review and editing, then committed as a draft (`status = 'draft'`) with its photographs. Publishing remains an explicit action on the piece page and fires the build hook.
- **Valuation research** (button on each piece's admin page): a server function has the model research comparable sales with web search, prioritizing Heritage Auctions results, eBay sold listings, and dealer catalogs, then a second, short structured-output request extracts the range and comparables (kept separate because citation-bearing search results and structured outputs are incompatible in one request). The research runs in a Netlify background function (`netlify/functions/ai-valuation-background.mts`, 15-minute budget; background mode is declared by the filename suffix, in-source config, and `netlify.toml` together, because in-source `config.background` alone was observed not to take effect) because real web-search research regularly exceeds the 60-second synchronous function limit. The piece page starts the run (the platform replies 202 immediately) and polls `/api/admin/pieces/[id]/valuations/ai-status`, which reads a run-state record in the Blobs `ai-status` store plus the newest `ai_research` row; the inserted row is the source of truth for completion. The background function lives outside the Astro app and background invocations are delivered asynchronously, so it does not rely on cookies or other browser headers surviving the queue: the authenticated dispatcher endpoint (`/api/admin/pieces/[id]/valuations/ai`, guarded by the admin middleware) owns the one-run-per-piece guard, records the `running` status, mints a short-lived signed run token, and invokes the worker with that token in the request body; the worker validates only the token. Every worker failure is recorded as a status instead of thrown, since Netlify retries a failed background invocation (which would re-run and re-bill the research). Research runs at medium effort and extraction at low effort. The result is stored as a new `valuations` row with method `ai_research`, including the comparables (source, description, price, date, URL) as JSONB. The piece page shows valuation history as a simple SVG chart, and AI-derived valuations are labeled as estimates everywhere they appear, including the CSV export.
- **Field companion** (`/admin/scout`): phone-first evaluation at a dealer's table. Photograph the piece (camera capture input), optionally type the asking price and where. One evaluation returns three results, streamed into cards as each completes: a duplicate check (photographs plus quick identification against the catalog's text; plausible matches come back with thumbnails and admin links), the same valuation research as piece valuations (dispatched to the background worker because of the 60-second synchronous limit; the scout page polls, and the result lives in the run's status record since nothing exists in the catalog yet), and a one-paragraph fit note referencing rooms and accessions, in the collection's voice. Nothing is written unless the owner taps save, which creates a **prospect**: accession `PROSPECT.<date>.<run>`, photographs moved from the ephemeral `scout/` blob keys to real piece keys, asking price and source captured in an owner research note, the fit note as an AI note, and the finished valuation (read server-side from the run record, never from the client) as an `ai_research` valuation row. Prospects never reach any public surface or the insurance CSV; the piece page offers Add to Collection as Draft when purchased.
- **Ask the curator** (question box in the walkthrough's exit section; endpoint `/api/curator/ask`): the one public AI feature. The server assembles the grounding context fresh per request from published content only (accessions, titles, meta lines, labels, room wall texts, and reviewed-and-public transcriptions), and the system prompt confines answers strictly to it, in the curatorial voice, citing accession numbers that the page renders as links to piece pages; when the collection holds nothing relevant the answer says so plainly. Single turn, no conversation memory, answers never stored as catalog content. Guardrails are durable in the `curator_questions` table: a per-visitor daily question limit (HTTP-only cookie plus IP; default 5, `CURATOR_DAILY_QUESTIONS`), a monthly token cap (default 2,000,000, `CURATOR_MONTHLY_TOKEN_CAP`) that closes the box with a graceful message until the month turns, and a response length cap from configuration. Every question and answer is logged with token usage and shown at `/admin/questions`, which is also a signal for what to acquire or write next.
- **Export** (`/admin/export`): downloads the full inventory as CSV for an insurance schedule: catalog record, acquisition data (date, source, price paid, provenance), latest valuation per piece (queried from the time series), latest condition grade, and image count. Prospects are excluded (not owned).
- **Connections** (intake commit, and a rescan button per piece): the model reads the piece plus the catalog's titles, labels, and reviewed transcriptions (prospects excluded) and proposes links: shared people, places, events, cause and effect, maker relationships. Each proposal is one grounded sentence in the collection's voice, stored unapproved in `piece_links` (directional in storage, bidirectional in display; the pair is unique). The piece page lists proposals with approve and reject, plus approved links with remove; rescanning replaces pending proposals and never touches approved ones. On public piece pages, related pieces are built from approved links first, each captioned with its reason, filling from same room then same type when fewer than three approved links exist; links touching unpublished pieces never render. Room placement suggestions come back with the intake proposal (the form preselects the suggestion; placement stays the owner's decision).
- **Transcription** (intake, per-piece button, and `/admin/transcriptions` backfill): the model transcribes all text on the object from its photographs (printed, manuscript, signatures, stamps, marginalia) with `[illegible]` for unreadable passages and `[?]` after doubtful readings, preserving original spelling and meaningful line breaks. Intake runs it in parallel with the proposal; the piece page can run or re-run it (re-running replaces the text and clears review); the backfill page queues checked pieces sequentially, one at a time, and reports per-piece results. AI output is always an unreviewed draft: the public piece page shows a transcription only when the owner marked it both reviewed and public, enforced inside the public data layer's query. Public transcriptions join the catalog's client-side search; admin search on the pieces list always includes transcriptions. One deliberate exception to the no-em-dash rule: transcriptions reproduce the object's own text exactly, so a dash printed on the object stays in its transcription.
...
- [x] **Phase 6** – addendum homepage copy; pieces move from `is_public` to a `status` enum (`prospect`/`draft`/`published`; existing rows mapped true to published, false to draft). Only `published` pieces reach public routes; prospects (field-companion saves, not owned) are also excluded from the insurance CSV.
- [x] **Phase 7** – document transcription: at intake, on demand per piece, and as a sequential backfill with per-piece results; reviewed flag plus public flag gate the public piece page; searchable in the catalog (public only) and admin (always); model limits centralized in `src/lib/ai-config.ts`.
- [x] **Phase 8** – connection detection: `piece_links` with one-sentence grounded reasons, proposed at intake commit and on demand, approve/reject on the piece page, related-pieces row built from approved links with reason captions, room suggestion in the intake proposal.
- [x] **Phase 9** – field companion at `/admin/scout`: duplicate check with candidate thumbnails, background valuation research, fit note, all streamed; save creates a prospect with notes and valuation attached.
- [x] **Phase 10** – ask the curator: public question box grounded strictly in published content with accession citations as links; durable daily rate limit and monthly token cap with a graceful closed message; Q&A log at `/admin/questions`.
